...
- name: Jordi Martorell-Marugán
  affiliation:
  - Computational Neuroscience. Joint Unit in Biomedical Imaging and Artificial Intelligence FISABIO–CIPF. Foundation for the Promotion of Health and Biomedical Research in the Valencian Region (Fisabio)
  email: jordi.martorell@fisabio.es
- name: Iván Ellson
  affiliation:
...
Several R packages implement molecular scoring methods, such as `decoupleR` (@badia2022) and `GSVA` (@hanzelman2013). `pathMED` unifies and simplifies these approaches, providing a common input format and allowing users to compute multiple scores efficiently. Additionally, `pathMED` includes our novel method, M-scores, which enhances disease characterization and prediction by leveraging reference datasets (@toroDominguez2022).
...
-   Z-score (@lee2018)
-   AUCell (@aibar2017)
-   decoupleR methods (MDT, MLM, ORA, UDT, ULM, WMEAN, norm_WMEAN, corr_WMEAN, WSUM, norm_WSUM and corr_WSUM) (@badia2022)
...
It may be overwhelming to novel users to make decisions about important parameters of the ML models training and validation processes. Therefore, here we list the most important parameters to tune and provide some advice:

-   **Koutter & Kinner in trainModel()**: These parameters control in how many folds the input data is split in the outer and inner nested cross-validation loops respectively. The default values (5 and 4) should work well as long as there are a sufficient amount of samples, but should be reduced otherwise. For instance, for 20 samples, the testing subsets of both the outer and the inner loops would contain only 4 samples (20/5 and 16/4 respectively). Although feasible, testing with only 4 samples may be missguiding hyperparameter optimization and validation. Lowering these parameters to 3 and 2 would results in testing subsets with 6-7 samples. On the opposite case, if there is a large amount of available samples, it may be useful to increase the default values to train more robust models, with the corresponding computational cost that should be also be considered.

-   **repeatsCV in trainModel()**: This is important for hyperparameter tuning. Although using the default (5) or increasing it may have some impact on the optimization, we recommend to lower this parameter (even to 1 to avoid CV iterations) if it is needed to reduce the computation time.

-   **tunelength in methodsML()**: This parameter controls how many random hyperparameters combinations will be tested during optimization. It is expected that increasing this value will improve the models performance, but implies longer computing time. In any case, we would not recommend to reduce the default value.
...
The genesets databases integrated in PathMED must be used following their corresponding data licenses. We list bellow their corresponding data licences or links where data use conditions are described, but users should verify potential use limitations in their corresponding webpages, especially regarding non-academic use.

-   Gene Ontology: Creative Commons Attribution 4.0 Unported License

-   Reactome: Creative Commons Public Domain Dedication (CC0)

-   KEGG: <https://www.pathway.jp/en/licensing.html>

-   tmod: GPL-2 \| GPL-3

-   Disgenet: <https://support.disgenet.com/support/solutions/articles/202000087487-do-i-need-a-commercial-license-can-i-use-disgenet-data-in-my-product->

-   Human Phenotype Ontology: <https://hpo.jax.org/license>

-   WikiPathways: Creative Commons CC0

-   PharmGKB: Creative Commons Attribution-ShareAlike 4.0 International License

-   LINCS: <https://lincsproject.org/LINCS/data/release-policy>

-   Comparative Toxicogenomics Database: <https://ctdbase.org/about/legal.jsp>
